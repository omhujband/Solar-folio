...
// ----- Planet WITH texture (useTexture always called) -----
function PlanetWithTexture({ color, radius, texture, ringTexture, hasRings }) {
...
// ----- Planet WITHOUT texture (no useTexture call) -----
function PlanetNoTexture({ color, radius, hasRings }) {
  const meshRef = useRef()
  const ringsRef = useRef()
...
// ----- Wrapper that chooses the right variant -----
  ...
  return (
    <group rotation={[0, 0, tilt]}>
      {texture ? (
        ...
        <PlanetNoTexture color={color} radius={radius} hasRings={hasRings} />
      )}
    </group>
  )
}
